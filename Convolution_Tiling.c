...
#pragma HLS ARRAY_PARTITION variable=out cyclic factor=8 dim=1
    ...
    // convolution
#pragma HLS UNROLL factor=TILING_FACTOR
for (int d_tile = 0; d_tile < OUT_DEPTH; d_tile += TILING_FACTOR) {
    for (int h_tile = 0; h_tile < OUT_HEIGHT; h_tile += TILING_FACTOR) {
        for (int w_tile = 0; w_tile < OUT_WIDTH; w_tile += TILING_FACTOR) {
            ...
            #pragma HLS ARRAY_PARTITION variable=tile_in cyclic factor=TILING_FACTOR dim=1
            ...
            #pragma HLS ARRAY_PARTITION variable=tile_out cyclic factor=TILING_FACTOR dim=1
            // Compute output tile
            for (int t = 0; t < TILING_FACTOR; t++) {
                int d = d_tile + t;
                if (d >= OUT_DEPTH) {
